...
if __name__ ==  "__main__":
    ...
    sc = axes[0][8].scatter(red_mut, np.array(bp_e_all)/6, c=navig_vals, cmap=cm, s=100, edgecolor="black")
    ...
    axes[0][8].set_ylabel("Base-pairing rule promiscuity", size=fontsize)
    axes[0][8].set_xlabel("Base-pairing rule degeneracy", size=fontsize)
    axes[0][8].set_xlim(-.06, 0.55)
    axes[0][8].set_ylim(0, 1.1)
    ...
    for i, idx in enumerate(new_order):
        ...
        if l == "canonical":
            axes[0][8].annotate(l, (red_mut[i]+0.01, (bp_e_all[i]/6)+0.025), size=15)
        elif l == "5":
            axes[0][8].annotate(l, (red_mut[i]-0.03, (bp_e_all[i]/6)+0.025), size=15)
        else:
            axes[0][8].annotate(l, (red_mut[i]+0.01, (bp_e_all[i]/6)+0.025), size=15)

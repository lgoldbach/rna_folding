...
if __name__ ==  "__main__":
    ...
    for idx, label in zip(new_order_idx, labels):
        ...
        for ph, c in zip(phenotypes, counts):
            if c > 0 and ph != "............":
                ...
                local_peak_x.append(rugged_av[ph]/folded_counts)
        ...
        local_peak_x_m = np.mean(local_peak_x)
        ...
        rxq1 = [local_peak_x_m-np.percentile(local_peak_x, q=25)]
        rxq2 = [max(0, np.percentile(local_peak_x, q=75)-local_peak_x_m)]
        ...
        ax2.errorbar(local_peak_x_m, y_m, xerr=(rxq1, rxq2), yerr=(yq1, yq2), elinewidth=1, marker="s", markersize=5, label=label)
        print(label, y_m)
        ...
        local_peak_x_m_all.append(local_peak_x_m)
        rug_x_m_all.append(rug_x_m)
        x_m_all.append(x_m)
        y_m_all.append(y_m)

    print(local_peak_x_m_all)
    r, p = pearsonr(local_peak_x_m_all, y_m_all)
    p_str = "%.3g" % p
    ax2.text(.4, .9, f'r = {np.round(r, 2)}\np = {p_str}', transform=ax2.transAxes, horizontalalignment='left', size=12)
    ...
    ax2.set_xlabel("Local peak frequency", size=15)
    ...
    ax2.legend(fontsize=9, title="GP map", loc="upper right", frameon=False)
    plt.tight_layout()

...
if __name__ ==  "__main__":
    ...
    for target_ph in phenotypes:
        ...
        for i in range(10):
            ...
            f_sum = 0
            count = 0
            for peak, f in zip(peaks_nc, peaks_f):
                if nc_graph.nodes[peak]["phenotype"] != target_ph:
                    f_sum += f * nc_graph.nodes[peak]["size"]
                    count += nc_graph.nodes[peak]["size"]
            if count > 0:
                f_av = f_sum/count
                peaks_height[target_ph].append(f_av)
            else:
                peaks_height[target_ph].append(-1)

    
    with open(args.output, "w") as f:
        for ph in peaks_height:
            f.write(ph + "\n")
            for height in peaks_height[ph]:
                f.write(str(height) + "\n") 

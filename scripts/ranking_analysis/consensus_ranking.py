...
### this is the more recent version of the script compared to "consensus_ranking_random.py" it seems
...
ranking = pickle.load(open("ph_ranking_ph_listlist.pickle", "rb"))
inconsistent = pickle.load(open("ph_inconsistent_ph_listtuple.pickle", "rb"))

ph_distr_vienn = open("phenotype_distr_vienna.txt", "r")
ph_distr_vienna = {}
for line in ph_distr_vienn:
    line_ = line.strip().split(" ")
    ph_distr_vienna[line_[0]] = int(line_[1])

gp_map_nussi7_folded_augc = pickle.load(open("gp_map_nussi7_augc_folded_dict.pickle", "rb"))

ph_distr_nussi7 = {}
for ph_list in gp_map_nussi7_folded_augc.values():
    ph_list = set(ph_list)
    for ph in ph_list:
...
# pickle.dump(ph_distr_nussi7, open("ph_distr_nussi7.pickle", "wb"))
ph_distr_nussi7 = pickle.load(open("ph_distr_nussi7.pickle", "rb"))

v_sum = sum(ph_distr_vienna.values())
ph_distr_vienna_norm = {ph: ph_distr_vienna[ph]/v_sum for ph in ph_distr_vienna}

n_sum = sum(ph_distr_nussi7.values())
ph_distr_nussi7_norm = {ph: ph_distr_nussi7[ph]/v_sum for ph in ph_distr_nussi7}

ph_sorted_nussi7 = [p for d, p in sorted(zip(ph_distr_nussi7.values(), ph_distr_nussi7.keys()), reverse=True)]

ranking_flat = [j for i in ranking for j in i]

y = [ph_distr_nussi7_norm[ph] for ph in ranking_flat]

# plt.scatter(range(len(ranking_flat)), y)
# plt.savefig("nussi_ph_freq_over_consensus_rank.pdf", format="pdf", dpi=15)

y2 = [ph_distr_vienna_norm[ph] for ph in ranking_flat]

plt.scatter(y2, y)
plt.xlabel("Phenotype frequency Vienna RNA")
plt.ylabel("Phenotype frequency suboptimal")
plt.savefig("Vienna_phdistr_vs_nussi_ph_distr.pdf", format="pdf", dpi=15)

plt.scatter(range(len(ranking_flat)), y2)
plt.savefig("vienn_ph_freq_over_consensus_rank.pdf", format="pdf", dpi=15)


consistency = [[] for i in range(A.shape[0])]
for i in range(A.shape[0]):
    for j in range(i+1,A.shape[1]):
        if A[i,j] == 0 and A[j,i] == 0:
            ...
            consistency[i].append(1)
            consistency[j].append(1)
        else:
            v = sorted([A[i,j],A[j, i]])
            consistency[i].append(1-(v[0]/v[1]))
            consistency[j].append(1-(v[0]/v[1]))
            # unbalanced_list.append(abs()/(A[i,j] + A[j, i]))

consistency_avg = [sum(i)/len(i) for i in consistency]

consistency_avg_sort = []
for ph in ranking_flat:
    consistency_avg_sort.append(consistency_avg[phenos_v.index(ph)])


plt.scatter(range(len(ranking_flat)), consistency_avg_sort)
plt.xlabel("Consensus Ranking")
plt.ylabel("Consistency of pairwise rankings")
plt.savefig("consens_rank_vs_rank_consistency.pdf", format="pdf", dpi=15)
